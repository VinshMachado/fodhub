...
router.post('/login',
    body('username').isLength({min:3,max:10}).withMessage('Your field must be between 3-10 characters long'),
    check('password').isLength({min:8,max:20}).withMessage('Your field must be between 8-20 charecters long'),
    async(request,response)=>{
    const {username,email}=request.body;
    const password=await hashPassword(request.body.password)
    const added=new User({"username":username,"password":password,"email":email})
